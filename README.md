...
1. 是否烧录了正确的固件？固件中ADC参考电压值是否与板子上的REF30xx一致？
2. 检查焊接，尤其注意DCDC，mos驱动，电流采样共模放大器的焊接。为了排除单片焊接问题，建议一次多焊接几片PCB。
3. 本项目手工焊接良品率可能不高，且BOM已经针对嘉立创SMT费用进行优化，**强烈建议**使用PCB厂家的SMT服务（可以只SMT被动元件，MCU等少数高价值元件手焊来节省成本）。
4. 电容组的满电电压是否和固件中定义的一致？
5. CAN接口在焊接时注意极性；立贴/卧贴的GH 2pin端口极性相反，请选择符合需求的版本。
